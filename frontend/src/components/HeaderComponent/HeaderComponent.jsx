        ...
        <div className='relative h-full flex items-center' onMouseOver={()=>handleMouseOver(t("Study24"))} onMouseLeave={()=>handleMouseLeave()}>
            ...
            <div className={`absolute top-full z-50 right-0 ${isHovered===t('Study24')?"block":"hidden"}`}>
                <div className="w-72 mt-1">
                    <BoxContent h1={t("Cho phép nhóm của bạn truy cập vào hơn 27.000 khóa học hàng đầu của Study24, ở mọi nơi và mọi lúc.")} textBn={t("Dùng thử Study24")} center={true} sizeH1={"19"}/>
                </div>
            </div>
        </div>
        <div className='relative h-full flex items-center' onMouseOver={()=>handleMouseOver(t('Giảng dạy trên Study24'))} onMouseLeave={()=>handleMouseLeave()}>
            <Link to="/teacher" className='px-3 hover:text-eighth tablet:hidden'>{t('Giảng dạy trên Study24')}</Link>
            <div className={`absolute top-full z-50 right-0 ${isHovered===t('Giảng dạy trên Study24')?"block":"hidden"}`}>
                <div className="w-72 mt-1">
